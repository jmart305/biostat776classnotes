...
ggplot(chocolate_percent_levels, aes(percent_group,rating, color = percent_group)) +
  ...
  theme(axis.text.x = element_text(angle = 60, vjust = 0.5)) + xlab("Chocolate Percentage") + ylab("Rating") + ggtitle("Ratings by Chocolate Percentage Groups for Countries with at least 50 Reviews") + labs(color='Percent Group') 
```

It seems that the 60-70% range has the highest ratings, but in a few countries, the 70-90% range ranks the highest. For example, in Belize, Nicaragua, Brazil, and Mexico, the 70-90% range ranks the highest, which is different from most of the other countries.
...
ggplot(chocolate, aes(country_of_bean_origin, company_location,color="red")) + 
  geom_point() + labs(title = "company location vs country of bean origin") + theme(title = element_text(size=2),axis.title.x = element_text(size=50), axis.title.y = element_text(size=50), panel.background = element_rect(fill = 'red2',color="yellow"),legend.position = "inside", strip.background=element_rect(fill="black"),strip.text = element_text(color="grey30")) + facet_wrap(~rating)
```
...
4. The color of the background makes it very hard to read the facet labels. The facet labels need to be easily visible as they display important information.  
5. The legend just reads "red" which doesn't give us any information and just makes more confusing.  
6. The plot is faceted by rating, but there are no labels to explain this so the view does not show what the plot is faceted by. It should be clear what the different facets are when using facet_wrap.  
...
chocolate %>%
   ...
   labs(subtitle = str_wrap("White points indicate the mean rating for each year; ratings have gone up slighly over time"))
```
...
4. I changed the color to be something on theme for chocolate and got rid of the legend because I don't feel that color scaling in this plot added any useful information. This also makes the whole graph wider which makes the violin plots easier to read.    
...
7. I made the x axis labels (years) tilted so that they are slightly easier to read.  
